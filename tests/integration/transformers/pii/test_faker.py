...
from rdt.transformers.pii import PIIFaker


def test_piifaker():
    """End to end test with the default settings of the ``PIIFaker``."""
    data = pd.DataFrame({
        'id': [1, 2, 3, 4, 5],
        'username': ['a', 'b', 'c', 'd', 'e']
    })
...
def test_piifaker_custom_provider():
    """End to end test with a custom provider and function for the ``PIIFaker``."""
    data = pd.DataFrame({
        'id': [1, 2, 3, 4, 5],
        'username': ['a', 'b', 'c', 'd', 'e'],
        'cc': [
    ...
    instance = PIIFaker('credit_card', 'credit_card_number')
...
def test_piifaker_with_nans():
    """End to end test with the default settings of the ``PIIFaker`` with ``nan`` values."""
    data = pd.DataFrame({
        'id': [1, 2, 3, 4, 5],
        'username': ['a', np.nan, 'c', 'd', 'e']
    })

    instance = PIIFaker(model_missing_values=True)
    transformed = instance.fit_transform(data, 'username')
    reverse_transform = instance.reverse_transform(transformed)
...
def test_piifaker_custom_provider_with_nans():
    """End to end test with a custom provider and function for the ``PIIFaker`` with `` nans``."""
    data = pd.DataFrame({
        'id': [1, 2, 3, 4, 5],
        'username': ['a', 'b', 'c', 'd', 'e'],
        'cc': [
