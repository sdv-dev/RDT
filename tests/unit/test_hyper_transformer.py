...
class TestHyperTransformer(TestCase):
    ...
    def test_fit_crashes(self):
        """Test it crashes when different transformer instances produce the same column name.
        ...
        """
        # Setup
        id2 = IdentityTransformer()
        id2.output_properties = {'is_null': {'sdtype': 'float', 'next_transformer': None}}
        id1 = IdentityTransformer()
        id1.output_properties = {
            None: {'sdtype': 'float', 'next_transformer': id2},
            'is_null': {'sdtype': 'float', 'next_transformer': None}
        }

        ht = HyperTransformer()
        data = pd.DataFrame({'col': [1, 2, 3]})
        ht.set_config({'sdtypes': {'col': None}, 'transformers': {'col': id1}})

        # Run and Assert
        err_msg = re.escape(
            'The column names generated by the transformers must be unique. The following'
            'output columns were produced multiple times: {col.is_null}.'
        )
